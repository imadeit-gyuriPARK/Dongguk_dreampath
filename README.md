...
- `초등학교`라는 특정 학교를 가리키지 않는 토큰이 5건 집계에 섞여 있음 — GT 사전(`gt_schoolnames.csv`) 자체에 이런 값이 정식명으로 들어있는 것으로 보이는 데이터 품질 이슈로, 아직 정리 안 됨
...
- GT 사전 자체 데이터 품질 문제 : "초등학교"라는 특정 학교를 안 가리키는 값이 `gt_schoolnames.csv`에 정식명으로 들어있었음 → `is_structurally_valid_school_name`(학교명은 항상 `[2글자 이상 고유명사]+[학교급 접미사]` 형태라는 규칙)으로 검증해 5건 발견 후 재처리
- 위 과정에서 `extract_school_candidates_fallback`(가장 짧은 접미사 매칭)의 한계 발견: "서초중"→"서초", "이대부초"→"이대"처럼 짧은 접미사가 먼저 걸려 뒤 글자를 놓치는 경우가 있어서, 가장 긴 접미사 매칭 버전(`extract_school_candidates_longest`)을 추가 (기존 함수는 3.0 파이프라인용이라 안 건드림)
...
- `ans_count==2`의 "맨 뒤 언급이 진짜 학교" 규칙은 49건 관찰로 귀납한 것이지 검증된 규칙은 아님 — 추후 행이 더 많아진다면 llm 이용해야한다. 
- 약어 문제 
  : "서울공고"(축약형, GT 미매칭 → `"서울공고등학교"`로 그대로 채택)와 "서울공업고"(GT 정식명 매칭 → `"서울공업고등학교"`)가 같은 학교인데 표기가 갈려서 최종 집계에서 안 합쳐짐(각 17건).
    => 가설 1 : 겹치는 글자가 많을수록 약어일 확률이 높다.
      1. school candidate에서 단어간의 유사성을 따진다? (내용적인거 말고 순수하게 겹치는 단어를 순서를 지키는 부분열(subsequence) 매칭) 
      -> 약어를 만드는 기준이 여러개이므로 클러스터링 진행하기 위한? (이화여대 -> 이대, 서울공업고등학교 -> 서울공고, 이화여자대학교부속초등학교 -> 이대부초)
      2. 규칙적인 패턴이 있는지 확인 (대략 3가지 정도로 예상됨)
      3. 약어 사전 만들어서 같다고 판정
